...
// --定位报名div--
var h = $('.columns').height()/2
$('.columns').css({'position':'absolute','bottom':-h+'px'})

//---分组---
var sumWidth =0;
$(".wrap ul").find("li").each(function(){
sumWidth += $(this).innerWidth();
});
console.log(sumWidth);
var a = $('.wrap ul').css('width',sumWidth+'px');
console.log(a.width())
// ---tab---
$('.grouping>li').click(function() {
    var index = $('.a>li').index(this);
    $(this).addClass('offline').siblings().removeClass('offline');
    $('.box2>li').eq(index).addClass('active').siblings().removeClass('active');
})
...
var box = document.getElementById('box');
    ...
    $(window).scroll(function(){
        ...
        if(dmt-wid<=top+2){               //文件高度-可视高度<=滚动高度
                    // 模拟 ajax 获取数据
                    var datas = [
                        '<a href="player.html" class="swiper-slide item"><div class="slide"><div class="number">123号</div><img src="image/c.jpg"><p>张三小可爱</p><p>6号</p><p>zhangsan</p><span>投票</span></div></a>',
                        '<a href="player.html" class="swiper-slide item"><div class="slide"><div class="number">123号</div><img src="image/cs6.jpg"><b>7号</b><p>张三</p><p>396票</p><span>投票</span></div></a>'
                    ];
                    var box = document.getElementById('box');
